...
\item Using constructive solid geometry (CSG) techniques, union the brushes for each AABB together to form a combined brush, then build a special kind of binary space partitioning (BSP) tree using the faces of the combined brushes for all the AABBs (see \S\ref{sec:onionbsp}). This tree can be used to decide which parts of the world are accessible by agents of each possible size.
\item Collect the faces from every leaf of the BSP tree and separate them into buckets, one for each AABB (note that each face must have previously been annotated with the AABB to which it corresponds to make this possible). Filter TODO
\end{enumerate}
%
